...
class vrp_by_geopy:
    ...
    def drop_client_to_df(self,client):
        self._df.drop(index=client,columns=client)
    #
    ...
    #
    def add_clien_to_vrp_dict(self,client, cord):
        #
        m = []
        #
        dict_dis = self.add_client_to_df_dist_sorted(client, cord)
    ...
    #
    def vrp_sol_info_dict(self,vec_dis, data):
        info = {}
        for x,y in self._vrp_affec_sol.items():
            d = {}
            d['size'] = len(y['sol'])
            d['full_Distance'] = self.all_distance_dict(vec_dis, y['sol'], data)
            d['max_Distance'] = self.max_distance_vec(vec_dis, y['sol'], data)
            if(x in [list(y.keys())[0] for y in self._basket]):
                d['delet']=[list(y.values())[0] for y in self._basket if list(y.keys())[0] == list(x.keys())[0]]
            #
            info[x]=d
        #
    ...
    def __repr__(self):
        return f" the clients ==>{self.clients}\n" \
               ...
               f" the vrp sol info ==> {self.vrp_info} "
    #
...
if __name__ == '__main__':
    ...
    #//print(tour.add_clien_to_vrp_dict('abdouy',(12, 15)), '\n')
    #print(tour.vrp_sol_info(vec_dis_depo, df))
    print(tour.vrp_sol_info_dict(vec_dis_depo, df))
    #print(tour._basket)
    #